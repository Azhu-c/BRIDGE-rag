import os
os.environ.setdefault("TRANSFORMERS_CACHE", "/data2/zxa/model")
import sys
from pathlib import Path
sys.path.append('/data2/zxa/rag_final_dataset/finetune/nova_code')
...
def load_nova_model_and_tokenizer(model_name='/data2/zxa/new_em/Nova-1.3b-new-arm', base_tokenizer_name='deepseek-ai/deepseek-coder-1.3b-base'):
...
def asm2vector_nova(asm: str, nova_tokenizer, model, token_id, max_len: int = 1024):
    asm = asm.strip()
    ...
    with torch.no_grad():
        outputs = model(input_ids=input_ids, nova_attention_mask=nova_attention_mask, return_dict=True, output_hidden_states=True)
        h = outputs.hidden_states[-1]
        e = h[0][input_ids[0] >= token_id].mean(dim=0)
    return e.cpu().numpy().astype(np.float32)


def build_vector_index(input_json: str, output_dir: str = '.', model_name='/data2/zxa/new_em/Nova-1.3b-new-arm', base_tokenizer_name='deepseek-ai/deepseek-coder-1.3b-base'):
    ...
    _, nova_tokenizer, model, token_id = load_nova_model_and_tokenizer(model_name, base_tokenizer_name)
    asm_bb_embeddings = [asm2vector_nova(asm_text, nova_tokenizer, model, token_id) for asm_text in asm_bb_list]
    ...
    index_path = out / 'nova_bb.index'
    ir_path = out / 'nova_ir.pkl'
    asm_path = out / 'nova_asm.pkl'
